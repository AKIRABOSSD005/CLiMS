...
public function insert_data($array)
{
    ...
    try {
        ...
        // Step 1: Check if `fname` exists for the member
        $stmt = $connection->prepare("SELECT member_id, fname FROM member WHERE fname = ?");
        $stmt->execute([$first_name]);
        $member = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$member) {
            $notifications[] = "First name '$first_name' not found in the database.";
            $connection->rollBack();
        ...
        if (!$mname_result) {
            $notifications[] = "Middle name '$middle_name' does not match for member with first name '$first_name'.";
            $connection->rollBack();
            return $notifications; // Stop processing this row but continue for others
        }
        ...
        if (!$lname_result) {
            $notifications[] = "Last name '$last_name' does not match for member with first name '$first_name' and middle name '$middle_name'.";
            $connection->rollBack();
            return $notifications; // Stop processing this row but continue for others
        }

        // Step 4: Check Loan Existence for this member
        ...
        // Step 5: Check for duplicate entry in loan_history
        ...
        // Step 6: Update Loan Information
