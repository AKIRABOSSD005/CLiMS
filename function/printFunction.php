...
$query = "SELECT m.member_id, m.fname, m.mname, m.lname, m.membership_fee, 
          ...
          LEFT JOIN loan_summary_report r ON l.loan_id = r.loan_id
          WHERE 1=1";
...
// Add the loan_status condition (this ensures you're excluding loan_status = 2)
$query .= " AND l.loan_status != 2";  // Exclude records with loan_status = 2 without existing loan
...
// Add total for the last processed month
        ...
        // Format and set the report history (Start Loan Date)
        if (!empty($rowData['report_history'])) {
