...
class DataImport
{
   ...
   // private $username = "root";
    ...
    public function insert_data($array)
    {
        ...
        $dateString = str_replace(',', '', trim($array[6]));
        $dateObject = DateTime::createFromFormat('F j Y', $dateString);
        if ($dateObject === false) {
            echo "<script>alert('Invalid date format for $first_name $middle_name $last_name');</script>";
            return;
        }
        $updated_balance_history = $dateObject->format('Y-m-d');

        try {
            $connection = $this->connect();

            // Check if `fname` exists in the database
            ...
            if ($member) {
                ...
                // Fetch loan_id early to avoid repeating logic
                $stmt = $connection->prepare("SELECT loan_id FROM loan WHERE member_id = ?");
                $stmt->execute([$member_id]);
                $loan_id = $stmt->fetchColumn();

                if ($loan_id) {
                    // Check for `mname` match
                    ...
                    // If any name mismatch, set balance_update_status to 0
                    // Check for mismatches in first name, middle name, and last name
                    if (($first_name != $member['fname'] && $middle_name == $member['mname'] && $last_name == $member['lname']) ||
                        ($first_name == $member['fname'] && $middle_name != $member['mname'] && $last_name == $member['lname']) ||
                        ($first_name == $member['fname'] && $middle_name == $member['mname'] && $last_name != $member['lname']) ||
                        ($first_name != $member['fname'] && $middle_name != $member['mname'] && $last_name == $member['lname']) ||
                        ($first_name != $member['fname'] && $middle_name != $member['mname'] && $last_name != $member['lname'])
                    ) {
                        ...
                        $notifications[] = "Name mismatch detected for $first_name $middle_name $last_name. Balance update status set to 0.";
                        return $notifications;
                    }

                    // Proceed with other logic if name matches
                    ...
                    if ($loan_data) {
                        ...
                        $notifications[] = "Loan ID not found for $first_name $middle_name $last_name.";
                    }
                } else {
                    $notifications[] = "Loan ID not found for $first_name $middle_name $last_name.";
                }
            } else {
                $notifications[] = "First name mismatch for $first_name $middle_name $last_name.";
            }
        } catch (PDOException $e) {
