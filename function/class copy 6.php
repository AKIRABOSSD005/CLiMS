...
class DataImport
{
    ...
    public function insert_data($array)
    {
        ...
        try {
            ...
            // Check if `fname`, `mname`, and `lname` exist in the database
            $stmt = $connection->prepare("SELECT member_id, fname FROM member WHERE fname = ?");
            $stmt->execute([$first_name]);
            $member = $stmt->fetch(PDO::FETCH_ASSOC);
    
            if (!$member) {
                $notifications[] = "First name not found in the database for $first_name $middle_name $last_name.";
                return $notifications; // Stop processing if no match found
            }
            ...
            if (!$mname_result) {
                $notifications[] = "Middle name does not match for $first_name $middle_name $last_name.";
                return $notifications; // Stop processing if no match
            }
            ...
            if (!$lname_result) {
                $notifications[] = "Last name does not match for $first_name $middle_name $last_name.";
                return $notifications; // Stop processing if no match
            }
    
            // Proceed to loan update if all name checks passed
            $stmt = $connection->prepare("SELECT loan_id, updated_balance FROM loan WHERE member_id = ?");
            $stmt->execute([$member_id]);
            $loan_data = $stmt->fetch(PDO::FETCH_ASSOC);
    
            if (!$loan_data) {
                $this->updateLoanStatus($connection, $loan_data['loan_id'], 0); // Update balance_update_status to 0
            ...
            // Check for duplicate entry in loan_history
            ...
            if ($existing_record > 0) {
                $this->updateLoanStatus($connection, $loan_id, 0); // Set balance_update_status to 0
                $notifications[] = "Duplicate entry for $first_name $middle_name $last_name with loan_id $loan_id on $updated_balance_history.";
                $connection->rollBack(); // Rollback if duplicate entry
                return $notifications;
            }
            ...
            // Debugging: Check if any row is updated
            if ($stmt_update->rowCount() > 0) {
                echo "Updated successfully."; // Debugging
            } else {
                echo "No rows were updated."; // Debugging
            }
            ...
            if ($stmt_update->rowCount() > 0) {
                ...
                // Set balance_update_status to 0 if update fails
                $this->updateLoanStatus($connection, $loan_id, 0);
...
//    // Define the updateLoanStatus method below:
    private function updateLoanStatus($connection, $loan_id, $status) {
     // Update the balance_update_status for the loan
     $stmt = $connection->prepare("UPDATE loan SET balance_update_status = ? WHERE loan_id = ?");
