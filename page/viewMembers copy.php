...
<body>
                              ...
                              WHERE m.member_id = '$id'";
                                ...
                                ?>
                                    <form action="../function/addBalanceFunction.php" method="POST">
                                        ...
                                        <input type="text" id="capitalBuildUp" name="capitalBuildUp">
                                        ...
                                        if (isset($_GET['id'])) {
                                            ...
                                            if ($result && mysqli_num_rows($result) > 0) {
                                                ...
                                                echo "Loan record(s) found for member_id: $member_id<br>";
                                            } else {
                                                echo "No loan records found for member_id: $member_id<br>";
                                            }
                                        } else {
                                            echo "member_id not set in the URL.<br>";
                                        }
                                        ?>
    ...
    const serverData = {
        loanDataExists: <?php echo $loan_data ? 'true' : 'false'; ?>,
        capitalBuildUp: <?php echo isset($capitalBuildUp) ? $capitalBuildUp : 0; ?>,
        loanId: <?php echo isset($loan_id) ? $loan_id : 'null'; ?>
    };



        // If loan records exist, make share_capital read-only and assign capitalBuildUp
        if (loanDataExists) {
            document.getElementById('share_capital').readOnly = false;
            document.getElementById('share_capital').value = '₱' + capitalBuildUp.toFixed(2);  // Assign formatted capitalBuildUp
            // Also set the capitalBuildUp field
            document.getElementById('capitalBuildUp').value = '₱' + capitalBuildUp.toFixed(2);
        } else {
            document.getElementById('share_capital').readOnly = true;
        }
        // Check if loan_id exists
    if (loanId !== null) {
        // Handle processing fee and interest calculations for an existing loan
        calculateLoanableAmount();
    } else {
        // Handle no loan data case, possibly reset input fields
        document.getElementById('processing_fee').value = '0.00';
        document.getElementById('interest').value = '0.00';
    }
        // Call calculateLoanableAmount to ensure fields are updated on page load
        calculateLoanableAmount();
    });
    </script>
    ...
    function calculateLoanableAmount() {
        ...
        // Update the 'share_capital' field, even if it is read-only
        document.getElementById('share_capital').readOnly = true; // Keep it read-only
        document.getElementById('share_capital').value =  capitalBuildUp.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
