                                    ...
                                    <div class="table-responsive" style="max-height: 395px; overflow-y: auto;">
                                        <div class="d-flex justify-content-between sticky-fix-top">
                                            <input type="text" id="searchInput" onkeyup="searchTable()"
                                                placeholder="Search for date..." class="form-control mb-3"
...
?>
                                        ...
                                        <div class="row mt-3" style="padding: 10px 0;">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control" id="share_capital" name="share_capital"
                                                    ...
                                                    style="border: 1px solid #388e3c; border-radius: 4px; padding: 10px;" required>
                                            </div>
                                            <div class="col-md-6">
    ...
    <input type="text" class="form-control" id="processing_fee" 
        ...
        style="border: 1px solid #388e3c; border-radius: 4px; padding: 10px;">
    <input type="hidden" id="hidden_processing_fee" name="processing_fee">
</div>
<div class="col-md-6 mt-3">
    ...
    <input type="text" class="form-control" id="interest" 
        ...
        style="border: 1px solid #388e3c; border-radius: 4px; padding: 10px;">
    <input type="hidden" id="hidden_interest" name="interest">
</div>
                                            ...
                                            style="border: 1px solid #388e3c; border-radius: 4px; padding: 10px;">
                                            <input type="hidden" id="net_proceeds_hidden" name="net_proceeds" value="">
                                            </div>


                                            <div class="col-md-6 mt-3">
                                                <!-- <label for="existing_balance" class="form-label">Existing Balance</label> -->
                                                <input type="hidden" id="existing_balance" name="existing_balance" class="form-control" style="border: 1px solid #388e3c; border-radius: 4px; padding: 10px;"
...
document.addEventListener('DOMContentLoaded', function () {
    // Get the loan data existence status from PHP
    const loanDataExists = <?php echo json_encode($loan_data_exists); ?>;
    console.log('loan_data_exists:', loanDataExists);  // Check if loan data exists

    // Get the necessary input fields
    ...
    function updateCalculations() {
        // Get and sanitize values from input fields
        ...
        let loanableAmount = principalAmount * 3;
        ...
        // If loan data exists, set the value of share_capital to the formatted capitalBuildUp
        if (loanDataExists) {
            console.log("Loan Data Exists");

            // Set the shareCapitalField to read-only and display the formatted capitalBuildUp value
            if (shareCapitalField) {
                shareCapitalField.readOnly = true;  // Make it read-only
                shareCapitalField.value = Math.max(capitalBuildUp, 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

                console.log('Share Capital Value Set:', shareCapitalField.value);  // Check if it's set
            }
        } else {
            console.log("No loan data exists.");

            // If no loan data exists, make the shareCapitalField editable
            if (shareCapitalField) {
                shareCapitalField.readOnly = false;  // Make the field editable
                shareCapitalField.value = '';  // Clear the field for input
            }
        }
    }

    // Attach event listeners to the input fields that affect the calculation
    principalAmountField.addEventListener('input', updateCalculations);
    existingBalanceField.addEventListener('input', updateCalculations);
    loanDurationField.addEventListener('input', updateCalculations);
...
function calculateLoanableAmount() {
    ...
    const capitalBuildUpField = document.getElementById('capitalBuildUp');
    const interestField = document.getElementById('interest');
    const hiddenProcessingFeeField = document.getElementById('hidden_processing_fee');
    const hiddenInterestField = document.getElementById('hidden_interest');
    const netProceedsHiddenField = document.getElementById('net_proceeds_hidden'); 
  ...
  let processingFee = calculateProcessingFee(principalAmount);
    processingFeeField.value = `₱${processingFee.toLocaleString()}`;
    hiddenProcessingFeeField.value = processingFee; 

    // Calculate interest
    let interest = updateInterest(principalAmount, loanDuration);
    interestField.value = `₱${interest.toLocaleString()}`;
    hiddenInterestField.value = interest; 
    ...
    let totalAmount = principalAmount - existingBalance - interest - processingFee - capitalBuildUp;
    totalAmountField.value = Math.max(totalAmount, 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    netProceedsHiddenField.value = totalAmount;
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.querySelector("form[action='../function/addBalanceFunction.php']");
        const addButton = document.getElementById("addBalanceButton-ID");
        ...
        const toggleButtonState = () => {
            addButton.disabled = !validateForm();
        };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Listen for the input event to check when the user types
    shareCapitalField.addEventListener('input', function () {
        let value = shareCapitalField.value;
        ...
        if (decimalCount > 1) {
            value = value.replace(/\.+$/, '');  // Remove extra decimal point if more than one
        }
        ...
        shareCapitalField.value = value;
    });
});
</script>

<script>
    // Search Function for 4th Column
    function searchTable() {
        ...
        for (i = 1; i < tr.length; i++) {
            td = tr[i].getElementsByTagName("td")[3]; // 4th column (Contact Number)
            if (td) {
