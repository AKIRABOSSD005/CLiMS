...
table th {
    text-align: left;
    background-color: #17a34a; /* Light gray background for header */
    font-weight: bold;
    color: white;
}
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="table-responsive text-center" style="max-height: 400px; overflow-y: auto;">
                                <table id="memberTable" class="table-bordered mt-2 table-striped">
                                    ...
                                    <tbody>
                                        <?php
                                        require '../conn/dbcon.php';
                                        $query = "SELECT m.member_id, m.fname, m.mname, m.lname, l.updated_balance_history, l.share_capital, l.minus_wage, l.updated_balance, l.balance_update_status
                FROM member m
                INNER JOIN loan l ON m.member_id = l.member_id ORDER BY m.fname ASC"; // Adjusted query to fetch loan-related data
                                        ...
                                        if ($result_query->num_rows > 0) {
                                            while ($data = $result_query->fetch_assoc()) {
                                                ...
                                                // Determine row style based on balance_update_status
                                                $rowStyle = ($data['balance_update_status'] == 0) ? 'style="background-color: #f69697 !important; color: black !important;"' : '';
                                        ?>
                                                <tr <?= $rowStyle ?>>
                                                    ...
                                                    <td hidden> <?= htmlspecialchars($data['balance_update_status']); ?></td>
                                                </tr>
                                        <?php
                                            }
                                        } else {
                ...
                <div class="modal-body">
                    <form id="monthlyForm" action="../function/printFunction.php" enctype="multipart/form-data"
                        ...
                        <div class="modal-footer d-flex justify-content-center align-items-center">
                            <button class="btn btn-success ms-2" name="printReport">Print
                                Report</button>
...
document.getElementById('passwordForm').addEventListener('keydown', (event) => {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            confirmPasswordButton.addEventListener('click', async function(event) {
                ...
                try {
                    ...
                    // Handle specific response strings
                    if (result === 'success') {
                        // Close Confirm Password Modal and open Print Report Modal
                        const confirmModalInstance = bootstrap.Modal.getOrCreateInstance(confirmPasswordModal);
                        confirmModalInstance.hide();
                        ...
                        // Display specific error message and keep the modal open
                        switch (result) {
                            case 'incorrect_password':
                                errorMessage.textContent = 'Incorrect password. Please try again.';
                                break;
                        ...
                        // Focus the password field for re-entry
                        document.getElementById('passwordInput').focus();
                    }
                } catch (error) {
                    console.error('Error during password verification:', error);
                    ...
                    // Keep the modal open and focus the password field
                    document.getElementById('passwordInput').focus();
                }
            });
 ...
 <script>
    // Search Function for First 3 Columns
    function searchTable() {
        ...
        for (i = 1; i < tr.length; i++) {
            ...
            for (j = 0; j < 3; j++) { // Loop through the first 3 columns
                td = tr[i].getElementsByTagName("td")[j];
                if (td) {
                    txtValue = td.textContent || td.innerText;
                    if (txtValue.toUpperCase().indexOf(filter) > -1) {
                        found = true; // Match found in one of the first 3 columns
                        break;
                    }
                }
            }
