    ...
    ?>
                    ...
                    <div class="modal-body">
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            <form action="../function/addBalanceFunction.php" method="POST">
                                ...
                                <input type="hidden" id="existing_balance" name="existing_balance"
                                    value="<?php echo isset($data['updated_balance']) && $data['updated_balance'] !== null ? number_format($data['updated_balance'], 2) : '0.00'; ?>">
                                <input type="hidden" name="member_id" value="<?= $data['member_id']; ?>">
                                <input type="hidden" id="processing_fee_hidden" name="processing_fee">
                                <input type="hidden" id="capitalBuildUp" name="capitalBuildUp">
                                ...
                                <div class="row mt-3" style="padding: 10px 0;">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control"  name="loanable_amount"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                // Event listeners for changes in principal or loan duration
                document.getElementById('principal_amount').addEventListener('input', calculateLoanableAmount);
                document.getElementById('loan_duration').addEventListener('change', calculateLoanableAmount);
            });

            function calculateLoanableAmount() {
                let shareCapital = parseFloat(document.getElementById('share_capital').value.replace(/[^0-9.-]+/g, '')) || 0;
                let loanableAmount = shareCapital * 3;

                // Cap loanable amount to 100,000
                ...
                // Handle principal amount and processing fee calculation
                let principalAmount = parseFloat(document.getElementById('principal_amount').value.replace(/[^0-9.-]+/g, '')) || 0;
                let processingFee = 0;
                let feeRanges = [
                ...
                // Determine processing fee based on principal amount
                for (let i = 0; i < feeRanges.length; i++) {
                    let range = feeRanges[i];
                ...
                // Placeholder variables for existing balance and loan duration
                let existingBalance = parseFloat(document.getElementById('existing_balance').value.replace(/[^0-9.-]+/g, '')) || 0;
                let loanDuration = document.getElementById('loan_duration').value;

                // Call updateInterest to calculate interest
                let interest = updateInterest(principalAmount, loanDuration);

                // Compute capital build-up share capital (adjust formula as necessary)
                let capitalBuildUp = (principalAmount - existingBalance - interest - processingFee) * 0.05;

                // Compute total loanable amount after adjustments
                let totalAmount = principalAmount - existingBalance - interest - processingFee - capitalBuildUp;

                // Ensure totalAmount and capitalBuildUp are valid numbers
                if (isNaN(totalAmount)) totalAmount = 0;
                if (isNaN(capitalBuildUp)) capitalBuildUp = 0;

                // Update the processing fee display
                ...
                // Display Net Proceeds and Capital Build-up
                document.getElementById('totalAmount').value = '₱' + totalAmount.toLocaleString();
                document.getElementById('capitalBuildUp').value = '₱' + capitalBuildUp.toLocaleString();
            }

            // Function to update interest based on selected principal and loan duration
            function updateInterest(principalAmount, loanDuration) {
                // Fetch interest from computation_loan table
                const interestRates = {
                    '5 Months': { 5000: 225.00, 10000: 450.00, 15000: 675.00, 20000: 900.00, 25000: 1125.00, 30000: 1350.00, 35000: 1575.00, 40000: 1800.00, 45000: 2025.00, 50000: 2250.00, 55000: 2475.00, 60000: 2700.00, 65000: 2925.00, 70000: 3150.00, 75000: 3375.00, 80000: 3600.00, 85000: 3825.00, 90000: 4050.00, 95000: 4275.00, 100000: 4500.00 },
                    '10 Months': { 5000: 413.00, 10000: 825.00, 15000: 1238.00, 20000: 1650.00, 25000: 2063.00, 30000: 2475.00, 35000: 2888.00, 40000: 3300.00, 45000: 3713.00, 50000: 4125.00, 55000: 4538.00, 60000: 4950.00, 65000: 5363.00, 70000: 5775.00, 75000: 6188.00, 80000: 6600.00, 85000: 7013.00, 90000: 7425.00, 95000: 7838.00, 100000: 8250.00 },
                    '15 Months': { 5000: 600.00, 10000: 1200.00, 15000: 1800.00, 20000: 2400.00, 25000: 3000.00, 30000: 3600.00, 35000: 4200.00, 40000: 4800.00, 45000: 5400.00, 50000: 6000.00, 55000: 6600.00, 60000: 7200.00, 65000: 7800.00, 70000: 7800.00, 75000: 9000.00, 80000: 9600.00, 85000: 10200.00, 90000: 10800.00, 95000: 11400.00, 100000: 12000.00 },
                    '20 Months': { 5000: 788.00, 10000: 1575.00, 15000: 2363.00, 20000: 3150.00, 25000: 3938.00, 30000: 4725.00, 35000: 5513.00, 40000: 6300.00, 45000: 7088.00, 50000: 7875.00, 55000: 8663.00, 60000: 9450.00, 65000: 10238.00, 70000: 11025.00, 75000: 11813.00, 80000: 12600.00, 85000: 13388.00, 90000: 14175.00, 95000: 14963.00, 100000: 15750.00 }
                };

                let interest = interestRates[loanDuration] && interestRates[loanDuration][principalAmount] || 0;
                if (interest === 0) {
                    // You may want to keep a previous valid value if interest is 0
                    let currentInterest = parseFloat(document.getElementById('interest').value.replace(/[^0-9.-]+/g, '')) || 0;
                    document.getElementById('interest').value = currentInterest !== 0 ? '₱' + currentInterest.toFixed(2) : '₱0.00';
                } else {
        ...
        <script>
            ...
            let isDropdownClick = false; // Flag to track if dropdown was clicked
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(member => {
                                ...
                                item.textContent = member.full_name; // Use the combined full_name
                                item.style.cursor = 'pointer'; // Indicate clickable dropdown
                                item.addEventListener('mousedown', () => {
                                    // Set the flag to prevent blur validation
                                    isDropdownClick = true;

                                    // Set input and hidden values
                                    document.getElementById(inputId).value = member.full_name;
                                    document.getElementById(hiddenId).value = member.full_name;

                                    dropdown.style.display = 'none';
                                    checkDuplicateNames(inputId, member.full_name); // Check for duplicate selection
                                });
            ...
            function validateInput(inputId, hiddenId) {
                // Skip validation if dropdown was clicked
                if (isDropdownClick) {
                ...
                if (!validNames[inputId] || !validNames[inputId].includes(inputField.value)) {
                    ...
                    hiddenField.value = inputField.value; // Set the hidden field if valid
                    checkDuplicateNames(inputId, inputField.value); // Check for duplicate selection
                }
            }
            ...
            document.getElementById('comaker_1').addEventListener('blur', () => validateInput('comaker_1', 'comaker_1_hidden'));
            document.getElementById('comaker_2').addEventListener('blur', () => validateInput('comaker_2', 'comaker_2_hidden'));
