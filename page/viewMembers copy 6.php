...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Check if loan data exists and if loanData is an object and contains capitalBuildUp
    if (loanDataExists && loanData && loanData.capitalBuildUp !== undefined) {
        console.log("Loan Data Exists and Capital Build Up is Valid");
        
        // Loan data exists and capitalBuildUp is a valid number
        const capitalBuildUpValue = parseFloat(loanData.capitalBuildUp);
        
        if (!isNaN(capitalBuildUpValue)) {
            // Format and set the share capital field to read-only
            const formattedCapital = `₱${capitalBuildUpValue.toFixed(2)}`;
            shareCapitalField.readOnly = true;
            shareCapitalField.value = formattedCapital;  // Set the value of share_capital
            capitalBuildUpField.value = formattedCapital;  // Set the value of capitalBuildUp
        } else {
            // If capitalBuildUp is invalid, set both fields to '₱0.00'
            shareCapitalField.value = '₱0.00';
            capitalBuildUpField.value = '₱0.00';
        }
    } else {
        console.log("No loan data exists or capitalBuildUp is missing.");
        
        // No loan data exists - allow user input and clear fields
        shareCapitalField.readOnly = false;  // Make the share capital input editable
        shareCapitalField.value = '';  // Clear the share capital field for input
        capitalBuildUpField.value = '';  // Clear the capitalBuildUp field for input
    }
});
</script>
...
function calculateLoanableAmount() {
    ...
    // Check if loan data exists to decide if share capital should be read-only
    const condition = loanDataExists;  // Check if loan data exists to make the share capital read-only

    if (condition) {
        shareCapitalField.readOnly = true; // Make it read-only if loan data exists
        shareCapitalField.value = 
            (typeof capitalBuildUp === 'number' && !isNaN(capitalBuildUp)) 
            ? `₱${capitalBuildUp.toLocaleString(undefined, { 
                minimumFractionDigits: 2, 
                maximumFractionDigits: 2 
            })}` 
            : '₱0.00'; // Default value if capitalBuildUp is invalid
    } else {
        shareCapitalField.readOnly = false; // Make it editable if no loan data exists
        shareCapitalField.value = ''; // Clear the value for manual input
    }
