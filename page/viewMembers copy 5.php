...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Example values (replace these with actual PHP values)
    let principalAmount = 5000;  // Example, replace with actual
    let existingBalance = 0;   // Example, replace with actual
    let interest = 225;           // Example, replace with actual
    let processingFee = 200;      // Example, replace with actual
    ...
    // Calculate capitalBuildUp
    let capitalBuildUp = (principalAmount - existingBalance - interest - processingFee) * 0.05;
    console.log('Calculated Capital Build-Up:', capitalBuildUp);  // Check the calculated value

    // Get the element for share_capital input field
    const shareCapitalField = document.getElementById('share_capital');
    console.log('Share Capital Field:', shareCapitalField);  // This should show the field element in the console

    // If loan data exists, set the value of share_capital to the formatted capitalBuildUp
    if (loanDataExists) {
        console.log("Loan Data Exists");

        // Format the capitalBuildUp value using toLocaleString for currency formatting
        const formattedCapital = capitalBuildUp.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        console.log('Formatted Capital:', formattedCapital);  // Check the formatted value
        ...
        if (shareCapitalField) {
            ...
            shareCapitalField.value = `₱${Math.max(capitalBuildUp, 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;  // Set the value
